...
echo "TAG (from ImmortalWrt): $TAG"
echo "Commit (from ImmortalWrt): $REPO_COMMIT"
echo "------------------------------------------"

if [[ "$WORKFLOW_NAME" == "Netis-NX31" ]]; then
   echo ">>> Device detected: $WORKFLOW_NAME. Starting to execute ImmortalWrt-specific modifications"

   # The logic below always executes, as the workflow is configured for Netis-NX31
   ...
   BASE_URL="https://downloads.immortalwrt.org/releases/25.12.0/targets/mediatek/filogic/kmods/"
   ...
   if [ "$KERNEL_VERSION" = "$REMOTE_KERNEL_VERSION" ]; then
     ...
     echo "Kernel version in the cloned repository ImmortalWrt: $KERNEL_VERSION"
